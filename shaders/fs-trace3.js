...
// col.xyz = hsv( _HueStart + _HueSize * vNoiseVal * _NoiseOffset * 10., _Saturation,_Lightness) * mat;// * audio.xyz;//mat;
 ...
 // vec3 baseColor = texture2D(t_colormap , vec2( _HueStart + _HueSize * vNoiseVal * _NoiseOffset * 10.  * .1,0.)).xyz;
 ...
 // col =  (texture2D(t_main, vUv).xyz * baseColor * _Lightness * 3.+texture2D(t_colormap, vec2( _HueStart + m * 10.,0.)).xyz)* mat;// + texture2D(t_colormap, vec2( _HueStart + m,0.)).xyz;
 // col *= texture2D(t_colormap, vec2( _HueStart + m * 10.,0.)).xyz;

float l = length(vUv-vec2(.5)) ;
 vec2 pVec = perp(vUv-vec2(.5));
 vec4 col = texture2D(_MainTex,vUv + pVec * sin(time * 1. + l * 30.)*.05 );
 ...
 col.xyz = texture2D(t_colormap, vec2( sin( l * 30. -time * 2.) * .1 + .8,0.)).xyz;
  gl_FragColor = col;//vec4( col.xyz , 1. );
