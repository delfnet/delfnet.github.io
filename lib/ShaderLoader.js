...
var ShaderLoader = function (manager, pathToShaders, pathToChunks) {
...
ShaderLoader.prototype.loadShaderChunk = function (type) {
  ...
  fetchShaderFile(self.pathToShaders, shader, type)
    .then(({ text, type }) => {
      self.onShaderLoaded(text, title, type);
    })
    .catch(error => {
      console.log(shader)
...
function fetchShaderFile(pathToShaders, shader, type) {
  var url = `./${pathToShaders}${shader}`;
  console.log(url)
  return fetch(url)
